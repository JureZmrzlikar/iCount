        ...
        for row in reader:
            ...
            for item in row[1:]:
                ...
                if up_limit <= pos <= down_limit:
                    landmark_count[landmark_name] = landmark_count.get(landmark_name, 0) + score

    top_n_landmarks = [lmark for lmark, _ in sorted(landmark_count.items(), key=lambda x: x[1], reverse=True)[:top_n]]
